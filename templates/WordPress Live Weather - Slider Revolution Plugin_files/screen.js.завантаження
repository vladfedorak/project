...
jQuery(document).ready(function() {
	...
	jQuery('body').on('click','#addToCartButton_ext',function(){
		fbq('track', 'AddToCart', {
			content_ids: ['2'],
		...
		//jQuery(".md-close.button").click();
		//console.log("ext");
	});

	jQuery('body').on('click','#addToCartButton_reg',function(){
		fbq('track', 'AddToCart', {
			content_ids: ['1'],
			content_type: 'product',
			value: 25.00,
	...
	jQuery('body').on('click','.revsliderboxplacer',function() {	
		var newhref = jQuery('.revsliderbox').attr('href');
	...
	*/
	...
	/*function countElements() {
      jQuery('body').find('.tp-num').each(function(i) {
          var tc = jQuery(this);
          if (tc.data('orig')===undefined) 
             tc.data('orig',tc.text());
          var obj = new Object();
          obj.sv = 0,
          obj.ev = parseInt(tc.data('orig'),0);
      	  obj.start = 2;
      	  obj.speed = 10;
	      tc.html("0");

		  punchgs.TweenLite.to( obj,  obj.speed, { sv: obj.ev , delay:obj.start,ease:punchgs.Power1.easeInOut,onUpdate:function(){ tc.html(parseInt(obj.sv,0)); } } )
      });
    }

	
	var tpcountelement = jQuery(".tp-num").offset().top;
	jQuery(window).scroll(function(){
	  var y = jQuery(window).scrollTop();
	    if (y >= (tpcountelement-100)){
	      countElements();
	    }
	});*/
...
function checkEGGrid() {
	if (jQuery('#esg-grid-1-1').length==0) return false;
	if (jQuery(window).width()<993 && (egcstate=="new" || egcstate=="desktop")) {
	   		jQuery('#esg-grid-1-1').esredraw({space:10})
	   		egcstate="mobile";
	} else
	if (jQuery(window).width()>992 && (egcstate=="new" || egcstate=="mobile")) {
	   		jQuery('#esg-grid-1-1').esredraw({space:30})
	   		egcstate="desktop";
	}
}

/************************************************
	-	INITIALISE PREVIEW IMAGES IN MENU 	-
*************************************************/
function initMenuPreviews() {
	jQuery('.menu-preview').each(function() {
		var mp = jQuery(this),
			cube = mp.find('.menu-preview-item-wrapper'),
			sm = mp.closest('.sub-menu'),
			def = mp.find('.default-image'),
			h = mp.height(),
			timers = new Array();
		
		
		punchgs.TweenLite.set(mp,{overflow:"visible"});//,perspective:800,transformStyle:"flat",perspectiveOrigin:"50% "+(h/2)+"px"})
		punchgs.TweenLite.set(cube,{position:"absolute",top:0,left:0,width:"100%",height:"100%",z:(0-h/2),transformStyle:"preserve-3d",transformOrigin:"0"+(h/2)+"px"})		
		sm.find('>li').each(function(i) {			
			if (i!=0) jQuery(this).addClass("mpiw-sensor");
		});

		sm.find('.mpiw-sensor').each(function(i) {			
			
				var li = jQuery(this),
					a = li.find('a').text().trim(),
					//regexp = new RegExp(a,"gi"),
					refimg = 0;
					
				mp.find('.menu-preview-item').each(function() {
					var mpi = jQuery(this),
						b = mpi.data('id').trim();	
					if (a == b){
						//if (b.match(regexp)){
				...
				punchgs.TweenLite.set(def,{overwrite:"all",autoAlpha:1,rotationX:0,z:h/2,y:0});
				
				if (refimg!=0) {
					li.data('r',refimg);
					
					punchgs.TweenLite.set(refimg,{overwrite:"all",autoAlpha:0,rotationX:-90,z:0,y:(h/2)});

					var tlc = new punchgs.TimelineLite();
					li.on("mouseenter",function() {
						var ind = li.closest('ul').parent().index();
						clearTimeout(timers[ind]);
						var o = sm.find('.selected'),
							i = jQuery(this).data('r');
							cubeAnim(h,o,i,cube,tlc,mp);						
					});
					li.on("mouseleave",function() {
						var o = sm.find('.selected'),
							ind = li.closest('ul').parent().index();															
						clearTimeout(timers[ind]);
						timers[ind] = setTimeout(function() {
							cubeAnim(h,o,def,cube,tlc,mp);
						},150);
...
function cubeAnim(h,o,i,cube,tlo,mp) {
	if (i.hasClass("selected")) return false;
	i.addClass("selected");
	...
	tlo.kill();
	tlo.add(punchgs.TweenLite.set(i,{force3D:"true",overwrite:"all",transformPerspective:800,autoAlpha:0,scale:1,rotationX:0,z:h/2,y:0}),0);	
	tlo.add(punchgs.TweenLite.set(o,{force3D:"true",overwrite:"all",transformPerspective:800,autoAlpha:1,scale:1,rotationX:-90,z:0,y:h/2}),0);						
	punchgs.TweenLite.to(i,0.4,{force3D:"true",autoAlpha:1,ease:punchgs.Power3.easeOut});
	punchgs.TweenLite.to(o,0.4,{force3D:"true",autoAlpha:0,ease:punchgs.Power3.easeOut});
	tlo.add(punchgs.TweenLite.fromTo(cube,0.4,{force3D:"true",scale:1,overwrite:"all",transformPerspective:800,rotationX:90},{z:(0-h),rotationX:0.01,ease:punchgs.Power3.easeOut}));
	tlo.play();
}
...
function initAnchors() {
	//jQuery('a[href^=#]').addClass('onepager-link');

	var hash = window.location.hash.substr(1);

	jQuery('a').each(function() {		
		var t = jQuery(this);
		if (t.attr('href')!=undefined && t.attr('href').indexOf("#")>=0) {			
			if (t.closest('.rev_slider_wrapper').length>0) return;	
			jQuery(this).addClass('onepager-link');
		}
	});
	...
	if (anchorarray.length>0) {
		...
		jQuery(window).on("scroll.onepager",function() {
			...
			jQuery.each(anchorarray,function(index,obj) {
				...
				obj.button.closest('li').removeClass("current-menu-ancestor");
				obj.button.removeClass("current-menu-ancestor");				
				if ((parseInt(doctop,0)+parseInt(lastknowheaderheight,0))>=(parseInt(obj.top,0)+parseInt(obj.offset,0)))
					if (lastobj.href!==obj.href && lastobj.top<obj.top) 
						lastobj = obj;						
			});


			if (lastobj!=undefined) {
				var url = lastobj.href;				
				jQuery.each(anchorarray,function(index,obj) {
					if (obj.href===url) {
						obj.button.parent().addClass("current-menu-ancestor");
						obj.button.closest('li').addClass("current-menu-ancestor");
						obj.button.addClass("current-menu-ancestor");
					}
		...
		jQuery.each(anchorarray,function(index,obj){
			if ("#"+hash===obj.href) jQuery(this).click();
		});
	...
	//try {
		if (href != undefined) {
			...
			if (obj.href!=undefined && obj.target!=undefined && jQuery(obj.target).length>0) {
				...
				if (jQuery(this).parent().hasClass("current-menu-ancestor")) return false;
				if (jQuery(this).hasClass("current-menu-ancestor")) return false;
				...
				jQuery.each(anchorarray,function(index,o) {
					if (obj.href!==o.href) {
						o.button.parent().removeClass("current-menu-ancestor");
						o.button.removeClass("current-menu-ancestor");
					} else {
						o.button.parent().addClass("current-menu-ancestor");
						o.button.addClass("current-menu-ancestor");
					}
				});
...
function parallaxBGPosition() {
	var doc = jQuery(document);
		...
		jQuery.each(parallaxobjects,function(index,pobj) {
			...
			// VIDEO START/STOP			
			if (pobj.isvideo && (pobj.oftop - doctop-mainwinh)<-200 && (pobj.oftop - doctop+pobj.bgh)>50) {
...
function initParallaxBg() {
	...
	jQuery('.parallaxbg .rowbgimage').each(function(i) {
			...
			if (pobj.bgimg.parent().find("video").length>0) {				
...
function initMegaMenu() {
	...
	jQuery('#navigation .megamenu').hover(function() {
		punchgs.TweenLite.killDelayedCallsTo(shortHandleMegamenuUl);
		punchgs.TweenLite.delayedCall(0.3,shortHandleMegamenuUl,[jQuery(this),"show"]);
	},function() {
		punchgs.TweenLite.killDelayedCallsTo(shortHandleMegamenuUl);
...
function shortHandleMegamenuUl(li,handler) {
			...
			var newlef = 0 - (hiw_w/2 - (ww-liwo));

			

		/*	if (ulo+newlef+hiw_w > jQuery(window).width()) 
				newlef = newlef + (jQuery(window).width() - (ulo+newlef+hiw_w));*/
			...
			setTimeout(function() {
					...
					switch (handler) {
						...
						case "set":
							...
							ul.css({width:hiw_w+"px", left:newlef+"px"});
						...
						case "show":
							...
							ul.css({width:hiw_w+"px",left:newlef+"px",clearProps:"transform"});
							punchgs.TweenLite.to(inside,0.15,{autoAlpha:1,ease:punchgs.Power2.easeInOut,clearProps:"transform"});
							punchgs.TweenLite.to(ul,0.15,{autoAlpha:1,ease:punchgs.Power2.easeInOut,clearProps:"transform"});
						break;
						case "hide":
							punchgs.TweenLite.to(inside,0.1,{autoAlpha:0,ease:punchgs.Power3.easeInOut});
							punchgs.TweenLite.to(ul,0.1,{autoAlpha:0,ease:punchgs.Power3.easeInOut,onComplete:function() {
